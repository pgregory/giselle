...
static GLUquadricObj* toGLUquadricObj (lua_State *L, int index)
{
  GLUquadricObj **quad = (GLUquadricObj **)lua_touserdata(L, index);
  if (quad == NULL) luaL_typerror(L, index, "GLUquadricObj");
  return *quad;
}

static GLUquadricObj* checkGLUquadricObj (lua_State *L, int index)
{
  GLUquadricObj **pi, *quad;
  luaL_checktype(L, index, LUA_TUSERDATA);
  pi = (GLUquadricObj **)luaL_checkudata(L, index, "GLUquadricObj");
  if (pi == NULL) luaL_typerror(L, index, "GLUquadricObj");
  quad = *pi;
  if (!quad)
    luaL_error(L, "null GLUquadricObj");
  return quad;
}
...
static int luaglu_new_quadric(lua_State *L)
{
	...
	pi = (GLUquadricObj**)lua_newuserdata(L, sizeof(GLUquadricObj *));
	*pi = quad;
	luaL_getmetatable(L, "GLUquadricObj");
...
static int luaglu_sphere(lua_State *L)
{
	...
	{
		luaL_error(L, "incorrect argument to function 'glu.Sphere'");
		return 0;
	}
	gluSphere(quad,
		(GLdouble)lua_tonumber(L, 2), (GLint)lua_tonumber(L, 3), (GLint)lua_tonumber(L, 4));
...
static int luaglu_cylinder(lua_State *L)
{
	...
	{
		luaL_error(L, "incorrect argument to function 'glu.Cylinder'");
		return 0;
	}
	gluCylinder(quad,
		(GLdouble)lua_tonumber(L, 2), (GLdouble)lua_tonumber(L, 3), (GLdouble)lua_tonumber(L, 4),
		(GLint)lua_tonumber(L, 5), (GLint)lua_tonumber(L, 6));
...
static int luaglu_disk(lua_State *L)
{
	...
	if(!( lua_isnumber(L, 2) && lua_isnumber(L, 3) && lua_isnumber(L, 4)
		&& lua_isnumber(L, 5)))
	{
		luaL_error(L, "incorrect argument to function 'glu.Disk'");
		return 0;
	}
	gluDisk(quad,
		(GLdouble)lua_tonumber(L, 2), (GLdouble)lua_tonumber(L, 3), 
...
static int GLUquadricObj_gc (lua_State *L)
{
  GLUquadricObj* quad = toGLUquadricObj(L, 1);
  if (quad) gluDeleteQuadric(quad);
  return 0;
}

static const luaL_reg GLUquadricObj_meta[] = {
  {"__gc",       GLUquadricObj_gc},
  {0, 0}
};
...
int luaopen_luaglu(lua_State *L)
{
  ...
  luaL_newmetatable(L, "GLUquadricObj"); /* create metatable for GLUquadricObj,
											add it to the Lua registry */
  luaL_openlib(L, 0, GLUquadricObj_meta, 0);  /* fill metatable */
